...
class LRR_Field_Roles {
	...
	public function input( $field ) {
        ...
        echo '<div class="lrr-repeaters-field lrr-repeaters-user_role">';

            $this->_add_new_tpl($field);
            echo '<hr>';

            echo '<div class="lrr-repeater-field__roles-wrap">';
                foreach ($value['label'] as $key => $name) {

                    echo '<div class="lrr-repeater-field__row" data-key="' , $key ,'">';
                        echo '<span class="lrr-repeater-field__row_actions">
                                <a href="#0" class="js-lrr-delete-row"><span class="dashicons dashicons-no lrr-repeater-field__row_action"></span></a>                                   
                            </span>';
                        $this->_roles_tpl($field, $key, $value);

                        echo '<a href="#0" class="js-lrr-sort-row" title="Drag to reorder"><span class="dashicons dashicons-menu"></span></a><hr>';
                    echo '</div>';
                }
            echo '</div>';
	...
	public function _add_new_tpl( $field ) {
	    echo '<button type="button" class="js-lrr-add-new-redirect-rule button button-primary" data-name="', $field->input_name() , '">Add new role</button>';
    }
	public function _roles_tpl( $field, $key = '', $value = null ) {
        ...
        echo '<span class="lrr-repeater-field__roles">';
        echo 'Label ';
