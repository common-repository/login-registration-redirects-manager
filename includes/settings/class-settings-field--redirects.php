...
class LRR_Field_Redirects {
	...
	public function input( $field ) {
        ...
        echo '<div class="lrr-repeaters-field">';
            if ( $per_role ) {
                $this->_add_new_tpl($field);
                echo '<hr>';

                echo '<div class="lrr-repeater-field__roles-wrap lrr-redirects-field__roles-wrap">';
                    foreach ($value['redirect'] as $key => $redirect) {

                        if ('default' === $key) {
                            continue;
                        }
                        echo '<div class="lrr-redirects-field__row lrr-repeater-field__row" data-key="' , $key ,'">';
                            echo '<span class="lrr-repeater-field__row_actions">
                                    <a href="#0" class="js-lrr-delete-row"><span class="dashicons dashicons-no lrr-redirects-field__row_action"></span></a>                                   
                                </span>';
                            $this->_roles_tpl($field, $key, $value);

                            $this->_redirects_tpl($field, $key, $value);
                            echo '<a href="#0" class="js-lrr-sort-row" title="Drag to reorder"><span class="dashicons dashicons-menu"></span></a><hr class="hr-blue">';
                        echo '</div>';
                    }
                echo '</div>';
        ...
        if ( $per_role ) {
            echo '<script type="text/html" class="js-lrr-repeater-tpl" data-name="', $field->input_name(), '">';
                echo '<div class="lrr-redirects-field__row lrr-repeater-field__row" data-key="%key%">';
                    echo '<span class="lrr-repeater-field__row_actions">
                                        <a href="#0" class="js-lrr-delete-row"><span class="dashicons dashicons-no lrr-redirects-field__row_action"></span></a>
                                    </span>';
                    ...
                    if ($per_role) {
                        ...
                        echo '<a href="#0" class="js-lrr-sort-row" title="Drag to reorder"><span class="dashicons dashicons-menu"></span></a>';
                    }
                    $this->_redirects_tpl($field, '%key%');
                echo '<hr class="hr-blue">';
                echo '</div>';
            echo '</script>';
        }
	...
	public function _add_new_tpl( $field ) {
	    echo '<button type="button" class="js-lrr-add-new-redirect-rule button button-primary" data-name="', $field->input_name() , '">Add new rule</button>';
    }
	public function _roles_tpl( $field, $key = '', $value = null ) {
        ...
        echo '<span class="lrr-redirects-field__roles">';
        echo 'If role match ';

        echo '<select name="' . $field->input_name() . '[role_match][' . $key . ']" class="role-match">';
            printf('<option value="%s" %s>%s</option>', 'any_of', selected('any_of', $role_match, false), 'any of');
            printf('<option value="%s" %s>%s</option>', 'all', selected('all', $role_match, false), 'all');
        echo '</select>';


        echo '<select multiple rows=2 name="' . $field->input_name() . '[roles][' . $key . '][]" class="pretty-select">';
        foreach (LRR_Roles_Manager::get_wp_roles_flat() as $role_key => $role_name) {
...
//        echo '<select name="' . $field->input_name() . '[role_match][' . $key . ']" class="role-match">';
//        printf('<option value="%s" %s>%s</option>', 'and', selected('and', $role_match, false), 'And');
//        printf('<option value="%s" %s>%s</option>', 'or', selected('or', $role_match, false), 'Or');
//        echo '</select>';
//        echo ' the username match ';
//        echo '<input type="text" class="lrr-pretty-multi-tags" name="' . $field->input_name() . '[username_match][' . $key . '][]">';
//
//        echo '<br/>';
    ...
    public function _redirects_tpl( $field, $key = '', $value = null ) {
        ...
        echo '<span class="lrr-redirects-field__redirect">';
        ...
        echo '<select name="' . $field->input_name() . '[redirect_page][' . $key . ']" class="redirect-page">';
            ...
            foreach (LRR_Pages_Manager::_get_pages_arr() as $page_ID => $page_title) {
