...
if ( ! function_exists('lrr_is_pro') ) {
     ...
     * @return bool
     */
    function lrr_is_pro($required_version = false)
    {
        if (!class_exists("LRR_Pro") || !defined('LRR_PRO_VERSION')) {
            return false;
        }
        if (!$required_version) {
            return true;
        }
        return version_compare(LRR_PRO_VERSION, $required_version, '>=');
    }
}

if ( ! function_exists('lrr_setting') ) {
    /**
     ...
     */
    function lrr_setting($setting_slug, $do_stripslashes = false)
    {
        return LRR_Settings::get()->setting($setting_slug, $do_stripslashes = false);
    }
}

if (!function_exists('lrr_log')) {
    /**
     * @param string $label
     * @param string $data
     *
...
if (!function_exists('lrr_dismissible_notice')) {
    /**
    ...
    function lrr_dismissible_notice($key, $message, $type = 'info', $required_capability = 'manage_options', $save_state_to = 'option')
    {
...
if (!function_exists('lrr_wc_version_gte')) {
    /**
     ...
     * @return bool
     */
    function lrr_wc_version_gte($version)
    {
