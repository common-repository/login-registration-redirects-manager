 ...
 */
class LRR_PRO_Roles_Manager {

	public static function init ()
	{
		if ( ! lrr_setting('user_role/general/on') ) {
			return;
		}

		add_action( "lrr/pre_register_new_user", array(__CLASS__, 'pre_register_new_user__action'), 10 );
		add_action( "lrr/registration_successful", array(__CLASS__, 'registration_successful__action'), 8, 1 );
	}
	...
	static function pre_register_new_user__action() {
		...
		if ( false === $selected_role || '' === $selected_role ) {
			wp_send_json_error(array('message' => lrr_setting('messages/registration/no_user_role'), 'for'=>'user_role'));
		}
	...
	static function registration_successful__action($user_id ) {
		...
		$active_roles = LRR_Field_Roles::_corrected_value( lrr_setting('user_role/general/active_roles') );
    ...
    public static function get_active_roles_flat ()
    {
	    ...
	    //$exists_roles = LRR_Roles_Manager::get_wp_roles_flat();
        $active_roles = LRR_Field_Roles::_corrected_value( lrr_setting('user_role/general/active_roles') );
