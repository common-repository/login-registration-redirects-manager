<div class="wrap underdev-settings <?php echo $this->handle; ?>-settings">
	...
	<h1><?php _e( 'Settings', 'login-registration-logout-redirects-manager' ) ?></h1>

	<?php if ( empty( $sections ) ): ?>
		<p><?php _e( 'No Settings available at the moment', 'login-registration-logout-redirects-manager' ); ?></p>
	<?php else: ?>
			...
			<ul class="menu-list">
				...
				<?php foreach ( $this->get_sections() as $section_slug => $section ): ?>
					...
					if ( $section_slug !== 'license' ) {
					    ...
					    // MAX
                        // @since 2.01
                        $url = admin_url('options-general.php?page=lrr_api_manager_dashboard');
                    }
					?>
